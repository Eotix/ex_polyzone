...
local function addToComboZone(zone)
    if comboZone ~= nil then
        ...
        comboZone = ComboZone:Create({ zone }, { name = "ex_polyzone" })
        comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if leftZones ~= nil then
              for i = 1, #leftZones do
                TriggerEvent("ex_polyzone:exit", leftZones[i].name)
              end
            end
            if enteredZones ~= nil then
              for i = 1, #enteredZones do
                TriggerEvent("ex_polyzone:enter", enteredZones[i].name, enteredZones[i].data, enteredZones[i].center)
              end
            end
        end, 500)
    end
...
RegisterNetEvent("ex_polyzone:createCircleZone")
AddEventHandler("ex_polyzone:createCircleZone", function(name, ...)
  addCircleZone(name, ...)
end)
